# Village Observer V0.30 — 행정·도시 통합 · 가격 V2

Village Observer는 실제 주민(Person)의 생활·노동·이동·소비가 **Settlement → Nation → World** 변화로 이어지는 browser-based bottom-up 사회 시뮬레이션입니다.

V0.30은 V0.29에서 형성된 국내경제를 **도시·행정·재정 구조**와 연결하고, 식량·목재·석재의 현지가격을 실제 소비·예상수요·생산·물류 신호에 맞게 정교화하는 버전입니다.

저장 데이터 버전은 `0.30`, localStorage 키는 `village-observer-v0-30`입니다. V0.29 이하 세이브는 기존 fallback chain으로 읽은 뒤 V0.30 런타임 필드를 붙입니다.
...
인구가 성장하면 한 국가에 여러 행정 중심지가 생길 수 있습니다. 현재 구현은 국가 규모에 따라 최대 4개 중심지를 허용하며, 지나치게 가까운 중심지의 중복 생성을 억제합니다.
...
기본 월간 비율은 약 `0.15%`이며 행정등급이 높은 중심지는 약간 높은 효율을 가집니다.
...
- World save version: `0.30`
- localStorage: `village-observer-v0-30`
- V0.29 이하 fallback load 유지
- 기존 Building ID 유지
- 기존 Person / Settlement / Nation 개체 유지
- V0.30 미존재 필드는 load 시 기본값 생성
- V0.30 → V0.30 save/load roundtrip 지원

V0.30은 V0.29 세이브를 읽을 때 기존 `v29Economy`, Person `gold29`, 국내거래 및 도시화 상태를 유지하고 행정·가격 V2용 필드를 추가합니다.
...
# 13. V0.30 장기데이터 우선 관찰 항목

1. 국가 / Settlement / Person Gold 비중의 장기 수렴
2. `observedGoldSource30` / `observedGoldSink30` 누적 속도
3. Settlement별 시장 Gold 집중도와 1인당 Gold 차이
4. 행정세 도입 후 Nation Treasury가 지나치게 고갈되거나 과잉축적되지 않는지
5. 고밀도 국가의 Stone/Wood 조달과 housing strain 변화
6. 식량 총량은 충분하지만 local food access가 막히는 사례의 변화
7. 가격 V2가 생산지와 소비지 사이의 가격차를 자연스럽게 만드는지
8. 창고 증설만으로 목재·석재 가격이 급등하는 현상이 사라졌는지
9. Merchant Guild / Grand Market / Deep Quarry / Stoneworks 실제 등장 여부
10. 리오·델마 같은 저밀도 확장국의 job capacity 과잉이 계속되는지
11. 생활권과 행정권이 지나치게 동일하게 겹치지 않는지
12. 국가별 성향 차이가 유지되는지
13. 1,500 / 3,000 / 5,000 Person 성능

---

# 14. V0.31~V0.33 연결

V0.30 이후 예정 흐름은 다음과 같습니다.

- **V0.31 — 자원 일반화 · 철기경제**: 철광석 → 제련 → 철제품 및 범용 자원/가공 사슬
- **V0.32 — 군사사회**: 실제 Person 군인, 장비, 부대, 군사 유지비와 보급
- **V0.33 — 전쟁 V1**: 실제 지도 이동·보급·전투·사상자·점령

V0.30에서 형성된 행정·재정·지역시장 구조는 이후 철 생산지, 가공도시, 군수경제와 전쟁 보급의 기반으로 사용합니다.
