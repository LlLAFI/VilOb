# Village Observer V0.32E1

**릴리스명:** 후기 성능 안정화 1차 — Unified Housing Resolver · Cached Food Relay · Quote Board UI Stability  
**버전:** `0.32E1`  
**기준 버전:** `V0.32D5`  
**세이브 키:** `village-observer-v0-32e1`

V0.32E1은 V0.32E의 첫 패치다. D5 장기주행에서 90~94년 구간의 SIM 비용이 크게 상승했고, 별도 계측에서 Housing 관련 계산이 같은 시기에 크게 폭증했다. 또한 후기 National Food Relay가 새로운 HOT path로 올라오기 시작했으며, 국제시장 호가판의 `<select>`와 `<details>`가 live render마다 다시 생성되어 사용 중인 UI가 매 tick 초기화되는 버그가 확인됐다.

E1은 **AI의 국제교역 행동이나 가격 밸런스를 바꾸는 패치가 아니다.** 우선순위 0인 UI 버그와, 이미 존재하는 주거·식량 fallback의 중복 계산을 줄이는 데 집중한다. Trade Network Planner, 상업시설 역할 재설계, 목표 날짜 자동진행 등은 이후 E 패치에서 다룬다.
